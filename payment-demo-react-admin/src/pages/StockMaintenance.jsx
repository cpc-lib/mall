...
import { Alert, Button, InputNumber, Modal, Select, Space, Table, Tag, Upload, message } from 'antd'
...
export default function StockMaintenance() {
  ...
  return (
    <div>
      <h2 className="adm-page-title">批量库存维护</h2>
      <div className="adm-card">
        <div className="adm-card-head">
          <span className="adm-card-title">导入记录</span>
          <span className="adm-card-sub">点选记录 → 点 ＋ 或“编辑 Excel”在新页签全屏核对/编辑并保存 → 确认入库执行；已入库记录不可重复执行</span>
      ...
      {batchResult && (
        <div className="adm-card">
          <div className="adm-card-head"><span className="adm-card-title">批量结果</span><span className="adm-card-sub">成功 {batchResult.successCount} 条 / 失败 {batchResult.failCount} 条</span></div>
          <Table rowKey={(_, i) => i} dataSource={batchResult.results} columns={resultCols} pagination={false} size="small" />
        </div>
      )}
      <div className="adm-card">
        <div className="adm-card-head"><span className="adm-card-title">库存流水</span><span className="adm-card-sub">含成功与失败记录，分页查询</span></div>
