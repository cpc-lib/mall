// 管理后台会话存储（与用户商城隔离，避免同源时相互覆盖登录态）
const PREFIX = 'admin_'
const ACCESS_KEY = PREFIX + 'access_token'
const REFRESH_KEY = PREFIX + 'refresh_token'
const USER_KEY = PREFIX + 'user'

// 一次性迁移：优先承接上一版隔离前缀 payment_demo_admin_*（角色已归属）；
// 再承接最初版共享前缀 payment_demo_*（按角色判断，仅迁移 ROLE_ADMIN）
;(function migrateLegacyAuth() {
  if (localStorage.getItem(ACCESS_KEY)) return
  let token = localStorage.getItem('payment_demo_admin_access_token')
  let refresh = localStorage.getItem('payment_demo_admin_refresh_token')
  let userRaw = localStorage.getItem('payment_demo_admin_user')
  if (!token) {
    ...
    if (token) {
      let legacyUser = null
      try { legacyUser = JSON.parse(userRaw || 'null') } catch { legacyUser = null }
...
export function clearAuth() { localStorage.removeItem(ACCESS_KEY); localStorage.removeItem(REFRESH_KEY); localStorage.removeItem(USER_KEY) }
