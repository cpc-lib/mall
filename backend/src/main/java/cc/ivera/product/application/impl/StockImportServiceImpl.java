...
public class StockImportServiceImpl implements StockImportService {
    ...
    private final StockFileStorage fileStorage;
    private final ObjectMapper objectMapper;

    public StockImportServiceImpl(StockImportRepository stockImportRepository, ProductRepository productRepository,
                                  ProductStockService productStockService, StockFileStorage fileStorage,
    ...
    @Override
    @Transactional
    public Long createImport(String fileName, String fileBase64, List<StockAdjustLine> items) {
        byte[] fileBytes = decodeFile(fileBase64);
        validateItems(items);
        StockImport record = StockImport.createPending(
            fileName == null || fileName.trim().isEmpty() ? "批量库存调整.xlsx" : fileName.trim(),
            items.size(), toJson(items));
        stockImportRepository.insert(record);
        ...
        stockImportRepository.update(record);
        return record.getId();
    }

    @Override
    public List<StockImport> listImports() {
        List<StockImport> rows = stockImportRepository.findRecent(LIST_LIMIT);
        for (StockImport row : rows) row.setItems(fromItemsJson(row.getItemsJson()));
        return rows;
    }

    @Override
    public StockImport getImport(Long id) {
        StockImport record = stockImportRepository.findById(id);
        if (record == null) throw new BizException("导入记录不存在");
        record.setItems(fromItemsJson(record.getItemsJson()));
        return record;
    }
    ...
    @Override
    @Transactional
    public void saveImportFile(Long id, String fileBase64, List<StockAdjustLine> items) {
        StockImport record = stockImportRepository.findById(id);
        if (record == null) throw new BizException("导入记录不存在");
        // 仅 PENDING 可编辑（守卫 + CAS 双重，并发确认抢占时 CAS 返回 0）
        record.requirePendingForEdit();
        byte[] fileBytes = decodeFile(fileBase64);
        validateItems(items);
        if (!stockImportRepository.updatePendingItems(id, items.size(), toJson(items), new Date())) {
    ...
    @Override
    public StockAdjustResult confirmImport(Long id) {
        StockImport record = stockImportRepository.findById(id);
        if (record == null) throw new BizException("导入记录不存在");
        List<StockAdjustLine> items = fromItemsJson(record.getItemsJson());
        if (items.isEmpty()) throw new BizException("导入明细为空，请先在编辑中保存明细后再确认入库");
        // CAS 抢占：PENDING→CONFIRMED，防并发/重复执行
        if (!stockImportRepository.casConfirm(id, new Date())) {
            throw new BizException("该导入记录已确认入库，不能重复执行");
        }
        List<StockAdjustResult.Row> rows = new ArrayList<>();
        int successCount = 0;
        for (StockAdjustLine item : items) {
            StockAdjustResult.Row row = new StockAdjustResult.Row();
        ...
        StockAdjustResult body = new StockAdjustResult();
    ...
    private void validateItems(List<StockAdjustLine> items) {
        ...
        for (StockAdjustLine item : items) {
            ...
            if (productRepository.findById(item.getProductId()) == null) {
    ...
    private String toJson(List<StockAdjustLine> items) {
        try {
    ...
    private List<StockAdjustLine> fromItemsJson(String json) {
        try {
            return objectMapper.readValue(json, new TypeReference<List<StockAdjustLine>>() {
            });
        } catch (Exception e) {
