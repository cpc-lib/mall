...
import cc.ivera.user.interfaces.dto.UserStatusRequest;
import cc.ivera.user.interfaces.vo.UserVO;
...
public class AdminUserController {
    ...
    public AdminUserController(UserAccountRepository userRepository, StringRedisTemplate redisTemplate) {
    ...
    public R<PageVO<UserVO>> list(@RequestParam(defaultValue = "1") int page,
        ...
        long total = userRepository.countAdminUsers(keywordLike);
        List<UserAccount> users = userRepository.listAdminUsers(keywordLike, safeSize, (long) (safePage - 1) * safeSize);
    ...
    public R<UserVO> setStatus(@PathVariable Long id, @RequestBody UserStatusRequest body) {
        ...
        if (!CommonStatus.ENABLED.getType().equals(status) && !CommonStatus.DISABLED.getType().equals(status))
            throw new BizException("用户状态仅支持 ENABLED / DISABLED");
        UserAccount u = userRepository.findById(id);
        if (u == null) throw new BizException("用户不存在");
                ...
                if (values != null) {
                    for (String v : values) {
                        if (v != null) {
                            ...
                            try {
                                onlineIds.add(Long.valueOf(parts[0]));
