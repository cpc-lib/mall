...
import cc.ivera.payment.domain.enums.alipay.AliPayTradeState;
...
public class AliPayServiceImpl implements AliPayService {
    ...
    private String doTradeCreate(Long productId, Long paymentAppId) {
        try {
            ...
            OrderInfo orderInfo = orderInfoService.createOrReuseOrder(
                ...
                payConfig.getAppId(),
                PaymentConfigGateway.CHANNEL_ALIPAY
            );
            // V2：发起支付前创建支付单（临近过期拒绝）。
            cc.ivera.payment.domain.model.PaymentOrder paymentOrder = paymentOrderService.startPayment(orderInfo, PaymentConfigGateway.CHANNEL_ALIPAY);
    ...
    public ChannelOrderQueryVO queryAndSyncStatus(String orderNo) {
        ...
        vo.setChannelCode(PaymentConfigGateway.CHANNEL_ALIPAY);
    ...
    private PaymentAppConfig resolveAliPayConfig(Long paymentAppId) {
        ...
        if (!PaymentConfigGateway.CHANNEL_ALIPAY.equals(config.getChannelCode())) {
