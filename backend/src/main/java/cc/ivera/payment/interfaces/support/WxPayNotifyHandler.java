...
public class WxPayNotifyHandler {
    ...
    public WxPayNotifyHandler(PaymentConfigGateway paymentConfigLoader,
    ...
    public String handle(HttpServletRequest request,
        ...
        try {
            ...
            Map<String, Object> bodyMap = parseNotifyBody(body);
            ...
            return processWithLock(requestId, () -> {
                if (!validateRequestWithAnyVerifier(request, requestId, body)) {
                    log.error("通知验签失败");
                    throw new BizException("通知验签失败");
                }
                log.info("通知验签成功");
    ...
    private Map<String, Object> parseNotifyBody(String body) {
    ...
    private String processWithLock(String requestId, Runnable task, HttpServletResponse response) {
        try {
            task.run();
    ...
    private List<Verifier> resolveCandidateVerifiers() {
        ...
        for (PaymentAppConfig config : paymentConfigLoader.listAppConfigsByChannelCode(PaymentConfigGateway.CHANNEL_WXPAY)) {
