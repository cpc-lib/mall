...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    private boolean isPublic(String uri, String method) {
        ...
        if ((uri.equals("/api/payment-app/list") || uri.equals("/api/payment-app/channels") || uri.startsWith("/api/payment-app/list-by-channel/")) && "GET".equalsIgnoreCase(method))
            return true;
        if (uri.startsWith("/api/auth/") || uri.startsWith("/api/cart") || uri.startsWith("/api/checkout") || uri.startsWith("/api/refund-applies") || uri.startsWith("/api/admin/") || uri.startsWith("/api/order") || uri.startsWith("/api/user/")) return false;
        if (uri.startsWith("/api/payment-app") || uri.startsWith("/api/payment-channel") || uri.startsWith("/api/payment-config") || uri.startsWith("/api/reconciliation") || uri.startsWith("/api/refund-info"))
            return false;
        return true;
    }
