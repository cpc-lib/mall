...
import cc.ivera.mapper.LocalMessageMapper;
...
public class LocalMessageServiceImpl implements LocalMessageService {
    ...
    public void markConsumed(String bizType, String bizNo) {
        ...
        UpdateWrapper<LocalMessage> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("biz_type", bizType)
                .eq("biz_no", bizNo)
                .in("status", LocalMessage.STATUS_PENDING, LocalMessage.STATUS_SENT)
                .set("status", LocalMessage.STATUS_CONSUMED)
    ...
    private void publishOne(LocalMessage message) {
        try {
            ...
            rabbitTemplate.convertAndSend(
                    exchangeOf(message.getBizType()),
                    routingKeyOf(message.getBizType()),
    ...
    private void markSent(Long id) {
        ...
        updateWrapper.eq("id", id)
                .eq("status", LocalMessage.STATUS_PENDING)
                .set("status", LocalMessage.STATUS_SENT)
                .set("update_time", new Date());
    ...
    private void scheduleRetry(LocalMessage message, String reason) {
        ...
        if (nextRetryCount > MAX_RETRY_COUNT) {
            updateWrapper.set("status", LocalMessage.STATUS_FAILED);
            ...
            updateWrapper.set("status", LocalMessage.STATUS_PENDING)
                    .set("next_retry_time", new Date(now.getTime() + delayMs));
     ...
     */
    private Object deserializePayload(LocalMessage message) {
        return JsonUtils.toObject(message.getMessageContent(), payloadClassOf(message.getBizType()));
    }

    private Class<?> payloadClassOf(String bizType) {
        switch (bizType) {
            case LocalMessage.BIZ_TYPE_ORDER_CLOSE:
                return OrderCloseMessage.class;
            case LocalMessage.BIZ_TYPE_REFUND_SYNC:
                return RefundStatusSyncMessage.class;
            default:
                throw new IllegalArgumentException("未知的本地消息业务类型：" + bizType);
        }
    }

    private String exchangeOf(String bizType) {
        switch (bizType) {
            case LocalMessage.BIZ_TYPE_ORDER_CLOSE:
                return OrderCloseRabbitConfig.ORDER_CLOSE_EVENT_EXCHANGE;
            case LocalMessage.BIZ_TYPE_REFUND_SYNC:
                return RefundStatusSyncRabbitConfig.REFUND_STATUS_SYNC_EVENT_EXCHANGE;
            default:
                throw new IllegalArgumentException("未知的本地消息业务类型：" + bizType);
        }
    }

    private String routingKeyOf(String bizType) {
        switch (bizType) {
            case LocalMessage.BIZ_TYPE_ORDER_CLOSE:
                return OrderCloseRabbitConfig.ORDER_CLOSE_DELAY_ROUTING_KEY;
            case LocalMessage.BIZ_TYPE_REFUND_SYNC:
                return RefundStatusSyncRabbitConfig.REFUND_STATUS_SYNC_DELAY_ROUTING_KEY;
            default:
                throw new IllegalArgumentException("未知的本地消息业务类型：" + bizType);
        }
    }
