...
public interface StockImportService {
    ...
    Long createImport(String fileName, String fileBase64, List<ProductStockBatchAdjustRequest.Item> items);

    /** 最近导入记录（最多 50 条，含解析后的明细）。 */
    List<StockImportVO> listImports();

    /** 按 id 查询导入记录（含解析后的明细），不存在抛 BizException。 */
    StockImportVO getImport(Long id);
    ...
    void saveImportFile(Long id, String fileBase64, List<ProductStockBatchAdjustRequest.Item> items);

    /** 确认入库：CAS 抢占记录（PENDING→CONFIRMED）后按记录已保存明细逐条执行库存调整，返回逐条结果。 */
    BatchAdjustResultVO confirmImport(Long id);
}
