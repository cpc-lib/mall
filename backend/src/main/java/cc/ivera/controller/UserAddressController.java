...
public class UserAddressController {
    ...
    public R<ShippingAddress> create(@RequestBody ShippingAddress addr) {
        ...
        if (addr.getReceiverName() == null || addr.getReceiverName().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "收货人姓名必填");
        if (addr.getReceiverPhone() == null || addr.getReceiverPhone().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "收货人电话必填");
        if (addr.getProvince() == null || addr.getProvince().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "省份必填");
        if (addr.getCity() == null || addr.getCity().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "城市必填");
        if (addr.getDistrict() == null || addr.getDistrict().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "区县必填");
        if (addr.getDetail() == null || addr.getDetail().trim().isEmpty()) return R.error(ErrorCode.PARAM_ERROR, "详细地址必填");
