...
- Rejected/dropped messages are safe: DB-driven backstop jobs reconcile independently of MQ —
  `TimeoutOrderCloseScheduler` (unpaid order close, every 60s) and `RefundStatusSyncScheduler` (PROCESSING refunds,
  every 60s).
...
- [ ] Confirm no stale queue with incompatible arguments already exists. RabbitMQ rejects redeclaration when queue
  arguments differ.
