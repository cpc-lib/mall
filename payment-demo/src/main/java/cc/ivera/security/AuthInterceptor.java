...
import cc.ivera.exception.ErrorCode;
...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (authorization == null || !authorization.startsWith("Bearer ")) return reject(response, ErrorCode.UNAUTHORIZED, "未登录或 Access Token 缺失");
        try {
            AuthPrincipal principal = authService.verifyAccessToken(authorization.substring(7).trim());
            if (requiresAdmin(uri, request.getMethod()) && !"ROLE_ADMIN".equals(principal.getRole())) return reject(response, ErrorCode.FORBIDDEN, "需要管理员权限");
            if (isUserOnlyShopping(uri) && "ROLE_ADMIN".equals(principal.getRole())) return reject(response, ErrorCode.FORBIDDEN, "管理员账号不支持购物车与下单操作");
    ...
    private boolean isPublic(String uri, String method) {
        ...
        if ((uri.equals("/api/payment-app/list") || uri.equals("/api/payment-app/channels") || uri.startsWith("/api/payment-app/list-by-channel/")) && "GET".equalsIgnoreCase(method)) return true;
        if (uri.startsWith("/api/auth/") || uri.startsWith("/api/cart") || uri.startsWith("/api/checkout") || uri.startsWith("/api/refund-applies") || uri.startsWith("/api/admin/") || uri.startsWith("/api/order")) return false;
        if (uri.startsWith("/api/payment-app") || uri.startsWith("/api/payment-channel") || uri.startsWith("/api/payment-config") || uri.startsWith("/api/reconciliation") || uri.startsWith("/api/refund-info")) return false;
        return true;
    }
    private boolean requiresAdmin(String uri, String method) {
        ...
        if (uri.startsWith("/api/payment-app") || uri.startsWith("/api/payment-channel") || uri.startsWith("/api/payment-config") || uri.startsWith("/api/reconciliation") || uri.startsWith("/api/refund-info")) return true;
        return false;
    }
