...
public class LoginGuardService {
    ...
    public LoginGuardService(StringRedisTemplate redisTemplate) { this.redisTemplate = redisTemplate; }
    ...
    public void clear(String username, String ip) {
        clearUsername(username); clearIp(ip);
    }
    ...
    private void throwIfLocked(String lockKey, String messagePrefix) {
        try {
            if (!Boolean.TRUE.equals(redisTemplate.hasKey(lockKey))) return;
