package cc.ivera.service.impl.bill;
...
import cc.ivera.enums.bill.BillType;
...
import cc.ivera.mapper.bill.BillImportMapper;
import cc.ivera.mapper.bill.BillReconcileDiscrepancyMapper;
import cc.ivera.mapper.bill.BillRecordMapper;
...
import cc.ivera.service.bill.parser.ParsedBill;
import cc.ivera.service.bill.parser.WxTradeBillParser;
...
public class BillReconcileServiceImpl implements BillReconcileService {
    ...
    public BillReconcileServiceImpl(
            BillImportMapper billImportMapper,
            BillRecordMapper billRecordMapper,
            BillReconcileDiscrepancyMapper discrepancyMapper,
    ...
    public BillImport uploadBill(MultipartFile file, String billDate, String billType) {
        ...
        // 快速幂等路径：同文件直接返回原批次
        BillImport existingByHash = findByFileHash(fileHash);
    ...
    private BillImport doUploadInLock(String fileName, String billDate, String fileHash,
        ...
        // 锁内二次检查
        BillImport byHash = findByFileHash(fileHash);
        if (byHash != null) {
            return byHash;
        }
        ...
        return billImportMapper.selectById(billImport.getId());
    }
        ...
        try {
            return lockTemplate.execute(LOCK_KEY_PREFIX + billImport.getBillDate(),
                    LOCK_WAIT_MILLIS, LOCK_LEASE_MILLIS, () -> {
                        BillImport locked = billImportMapper.selectById(billImport.getId());
                        if (BillImportStatus.RECONCILED.getType().equals(locked.getStatus())) {
                            return locked;
                        }
                        reconcileInTransaction(locked.getId());
                        return billImportMapper.selectById(locked.getId());
                    });
        } catch (BizException e) {
            if (Objects.equals("系统繁忙，请勿重复提交", e.getMessage())) {
                throw new BizException("该账单正在对账处理中或已处理完成，请勿重复提交");
    ...
    private void reconcileInTransaction(Long importId) {
        transactionTemplate.executeWithoutResult(status -> {
            BillImport billImport = billImportMapper.selectById(importId);
            if (billImport == null) {
                throw new BizException("账单导入批次不存在");
            }

            discrepancyMapper.delete(new QueryWrapper<BillReconcileDiscrepancy>()
                    .eq("import_id", importId));

            List<BillRecord> records = billRecordMapper.selectList(new QueryWrapper<BillRecord>()
                    .eq("import_id", importId)
                    .orderByAsc("id"));
    ...
    private int reconcilePayRecords(BillImport billImport, List<BillRecord> payRecords,
        ...
        List<PaymentInfo> localDayPayments = paymentInfoMapper.selectList(new QueryWrapper<PaymentInfo>()
                .eq("payment_type", CHANNEL_WXPAY)
                .eq("trade_state", WxTradeState.SUCCESS.getType())
    ...
    private int reconcileRefundRecords(BillImport billImport, List<BillRecord> refundRecords,
        ...
        Date dayStart = atStartOfDay(billImport.getBillDate());
        Date dayEnd = atStartOfNextDay(billImport.getBillDate());
        List<RefundInfo> localDayRefunds = refundInfoMapper.selectList(new QueryWrapper<RefundInfo>()
                .eq("refund_status", RefundStatus.SUCCESS.getType())
                .ge("create_time", dayStart)
                .lt("create_time", dayEnd));
    ...
    private Map<String, PaymentInfo> loadLocalPayments(Set<String> orderNos) {
        ...
        List<PaymentInfo> payments = paymentInfoMapper.selectList(new QueryWrapper<PaymentInfo>()
                .eq("payment_type", CHANNEL_WXPAY)
                .in("order_no", orderNos));
        return payments.stream().collect(Collectors.toMap(PaymentInfo::getOrderNo, Function.identity(), (a, b) -> a));
    }
    ...
    private Map<String, RefundInfo> loadLocalRefunds(Set<String> refundNos) {
        ...
        List<RefundInfo> refunds = refundInfoMapper.selectList(new QueryWrapper<RefundInfo>()
                .in("refund_no", refundNos));
        return refunds.stream().collect(Collectors.toMap(RefundInfo::getRefundNo, Function.identity(), (a, b) -> a));
    }
    ...
    public List<BillImport> listImports(String billDate) {
        ...
        return billImportMapper.selectImportsByDate(billDateValue);
    }
    ...
    public List<BillRecord> listRecords(String importNo, String recordType) {
        ...
        return billRecordMapper.selectRecordsByImport(billImport.getId(), recordTypeValue);
    }
    ...
    public List<BillReconcileDiscrepancy> listDiscrepancies(String importNo, String bizType,
        ...
        return discrepancyMapper.selectDiscrepanciesByImport(
    ...
    public BillReconcileDiscrepancy resolveDiscrepancy(Long id, String resolveRemark) {
        ...
        BillReconcileDiscrepancy discrepancy = discrepancyMapper.selectById(id);
        if (discrepancy == null) {
    ...
    private BillImport requireByImportNo(String importNo) {
        ...
        BillImport billImport = billImportMapper.selectOne(new QueryWrapper<BillImport>()
                .eq("import_no", importNo.trim()));
        if (billImport == null) {
    ...
    private BillImport findByFileHash(String fileHash) {
        return billImportMapper.selectOne(new QueryWrapper<BillImport>().eq("file_hash", fileHash));
    }

    private List<BillImport> listByChannelDate(String billDate) {
        return billImportMapper.selectList(new QueryWrapper<BillImport>()
                .eq("channel_code", CHANNEL_WXPAY)
                .eq("bill_type", BillType.TRADE.getType())
    ...
    private void assertBillDateKindNotExists(String billDate, BillKind billKind) {
        List<BillImport> sameDate = listByChannelDate(billDate);
        if (sameDate.isEmpty()) {
            return;
        }
    ...
    private void validateBillDate(String billDate) {
        ...
        try {
            ...
            throw new BizException("账单日期格式必须为 yyyy-MM-dd");
        }
