...
import cc.ivera.security.LoginGuardService;
import cc.ivera.security.PasswordHasher;
...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthServiceImpl(UserAccountMapper userMapper, PasswordHasher passwordHasher, JwtTokenService jwtTokenService, StringRedisTemplate redisTemplate, LoginGuardService loginGuard) {
    ...
    public UserAccount register(RegisterRequest request) {
        ...
        catch (DuplicateKeyException e) { throw new BizException("用户名已存在", e); }
    ...
    public LoginVO refresh(String refreshToken) {
        ...
        if (user == null || !ENABLED.equals(user.getUserStatus())) throw new BizException("用户不可用");
    ...
    public void logout(AuthPrincipal principal, String refreshToken) {
        ...
        if (refreshToken != null && !refreshToken.trim().isEmpty()) redisTemplate.delete(REFRESH_PREFIX + sha256(refreshToken));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void changePassword(AuthPrincipal principal, PasswordChangeRequest request) {
        UserAccount user = userMapper.selectById(principal.getUserId());
        if (user == null || !passwordHasher.matches(request.getOldPassword(), user.getPasswordSalt(), user.getPasswordHash())) throw new BizException("原密码错误");
    ...
    private UserAccount findByUsername(String username) {
        QueryWrapper<UserAccount> q = new QueryWrapper<>(); q.eq("username", username == null ? null : username.trim());
        return userMapper.selectOne(q);
    }
    private void resetPassword(UserAccount user, String password) {
        String salt = passwordHasher.newSalt();
        user.setPasswordSalt(salt); user.setPasswordHash(passwordHasher.hash(password, salt));
    ...
    private void bumpVersion(Long userId) { redisTemplate.opsForValue().increment(VERSION_PREFIX + userId); }
    ...
    private UserAccount sanitize(UserAccount source) {
        UserAccount u = new UserAccount(); u.setId(source.getId()); u.setUsername(source.getUsername()); u.setRole(source.getRole()); u.setUserStatus(source.getUserStatus()); u.setCreateTime(source.getCreateTime()); u.setUpdateTime(source.getUpdateTime()); return u;
    }
    private String sha256(String input) {
        try { return Base64.getUrlEncoder().withoutPadding().encodeToString(MessageDigest.getInstance("SHA-256").digest(input.getBytes(StandardCharsets.UTF_8))); }
        catch (Exception e) { throw new BizException("Token 摘要失败", e); }
    }
}
